...
const ReservaTarj: React.FC = () => {
  ...
  const [selectedReserva, setSelectedReserva] = useState<PedidoReserva | null>(null);
  const [modalVisible, setModalVisible] = useState(false);
  const [modoEdicion, setModoEdicion] = useState(false);
  const [tabActiva, setTabActiva] = useState<"reservas" | "pagadas">("reservas");

  const [modoAccion, setModoAccion] = useState<"modificar" | "eliminar" | null>(null);
  ...
  const guardarCambios = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8084/api/pedidos/${selectedReserva.id}`, {
        method: "PUT",
        headers: {
  ...
  const reservasFiltradas = reservas.filter((reserva) =>
    tabActiva === "pagadas" ? reserva.estado === "pagada" : reserva.estado !== "pagada"
  );
    ...
    <div className="reserva-contenedor">
      ...
      <div className="tarjetas-grid">
        {reservasFiltradas.map((reserva, index) => (
          <div
            className={`reserva-mini ${selectedReserva?.id === reserva.id ? "seleccionada" : ""}`}
            key={index}
            onClick={() => handleClickReserva(reserva)}
          >
            <h3>{reserva.infoMenu.servicio.tipoServicio.nombre}</h3>
            <p><strong>Fecha:</strong> {reserva.datosEvento.fechaEvento}</p>
            <p><strong>Dirección:</strong> {reserva.datosEvento.direccion}</p>
            <span
            ...
            {modoEdicion ? (
              <>
                ...
                <div className="modal-botones">
                  <button className="btn-editar" onClick={guardarCambios}>Guardar</button>
