...
export const RegisterForm = ({ onCancelarClick }: RegisterFormProps) => {
  ...
  const { setIsAuthenticating, login } = useAuth(); // login will now only be called after verification
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(
          data.message || "Registro exitoso. Revisa tu correo para verificar."
        );
        // Redirect to the verification page, passing the email for convenience
